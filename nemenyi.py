...
def nemenyi_test_and_plot(results_file_path, ascending=False, save_plot=False):
    df = pd.read_csv(results_file_path, index_col="Dataset")
    result = df.to_numpy()

    num_datasets, num_algorithms = result.shape
    av_ranks = np.zeros(num_algorithms)

    for i in range(num_datasets):
        vals = result[i]
        ranks = scipy.stats.rankdata(vals, method='average')
        if not ascending:  # if higher values mean better
            ranks = len(ranks) + 1 - ranks  # invert ranks
        av_ranks += ranks

    av_ranks /= num_datasets

    order = np.argsort(av_ranks)
    names = df.columns[order].tolist()
    av_ranks = av_ranks[order]

    n = num_datasets
    cd = compute_CD(av_ranks, n, alpha=0.05, test="nemenyi")

    print("CD =", cd)
    print("Average ranks:", dict(zip(names, np.round(av_ranks, 3))))

    graph_ranks(av_ranks, names, cd=cd, cdmethod=None, width=8, textspace=1.5)
    if save_plot:
        plt.savefig("./results/nemenyi_plot_{}.eps".format(
            results_file_path.split("/")[-1].replace(".csv", "")), format='eps')

    plt.show()

# Nemenyi Test by the Orange3 library.
# It is included here since it was deprecated in the latest versions of Orange3
