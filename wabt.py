...
class WeightAdjustingBinaryTransformation:
    ...
    def _update_weights(self, y):
        ...
        for original_index, bit in enumerate(y_list):
            if bit == 1:
                self.label_weights[original_index] = min(self.label_weights[original_index] + self.increase_factor,
                                                         self.upper_weight_limit)
            else:
                self.label_weights[original_index] = max(self.label_weights[original_index] - self.decrease_factor,
                                                         self.lower_weight_limit)
